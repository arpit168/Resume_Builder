...
export function ResumeProgress({ data }: { data: ResumeData }) {
  const progress = useMemo(() => {
    ...
    if (data.experience.length > 0) {
      score += 15;
      if (data.experience[0].description && data.experience[0].description.length > 20) {
        score += 10;
      }
    }
    ...
    // Extras (Max 10%)
    if (data.projects.length > 0 || data.certifications.length > 0 || data.languages.length > 0) {
      score += 10;
    }
  ...
  return (
    <div className="w-full bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 p-4">
      ...
      <p className="text-xs text-gray-500 mt-2">
        ...
        {progress >= 40 && progress < 70 && "Looking good! Add some experience and skills."}
        {progress >= 70 && progress < 100 && "Almost there! Add a summary or some extra sections."}
