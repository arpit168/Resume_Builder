...
export function CustomSectionForm({ resume }: { resume: Resume }) {
  const { addCustomSection, updateCustomSection, deleteCustomSection, updateResume } = useResume();
  const customSections = resume.data.customSections;
  const [expandedId, setExpandedId] = useState<string | null>(customSections[0]?.id || null);
  ...
  const moveUp = (index: number) => {
    ...
    updateResume(resume.id, { data: { ...resume.data, customSections: newArr } });
  };
      ...
      {customSections.length === 0 ? (
        ...
        <div className="space-y-4">
          {customSections.map((section, index) => {
            ...
            return (
              <div key={section.id} className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden bg-white dark:bg-gray-800 transition-all">
                {/* Header */}
                <div 
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="flex flex-col gap-1" onClick={(e) => e.stopPropagation()}>
                      ...
                      <button onClick={() => moveDown(index)} disabled={index === customSections.length - 1} className="text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 disabled:opacity-30">
                  ...
                  <div className="flex items-center gap-2">
                    <button 
                      onClick={(e) => { e.stopPropagation(); deleteCustomSection(resume.id, section.id); }}
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) => updateCustomSection(resume.id, section.id, { content: e.target.value })}
