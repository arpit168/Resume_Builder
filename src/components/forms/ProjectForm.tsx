...
export function ProjectForm({ resume }: { resume: Resume }) {
  const { addProject, updateProject, deleteProject, updateResume } = useResume();
      ...
      {projects.length === 0 ? (
        ...
        <div className="space-y-4">
          {projects.map((project, index) => {
            ...
            return (
              <div key={project.id} className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden bg-white dark:bg-gray-800 transition-all">
                {/* Header */}
                <div 
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="flex flex-col gap-1" onClick={(e) => e.stopPropagation()}>
                      ...
                      <button onClick={() => moveDown(index)} disabled={index === projects.length - 1} className="text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 disabled:opacity-30">
                  ...
                  <div className="flex items-center gap-2">
                    <button 
                      onClick={(e) => { e.stopPropagation(); deleteProject(resume.id, project.id); }}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="md:col-span-2">
                        ...
                        <input
                          ...
                          onChange={(e) => updateProject(resume.id, project.id, { technologies: e.target.value })}
                          placeholder="e.g. React, Node.js, MongoDB"
                          className="w-full border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2 bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        />
                      </div>
                      <div>
                        <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Live URL (Optional)</label>
                        <input
                          type="url"
                          value={project.projectUrl}
                          onChange={(e) => updateProject(resume.id, project.id, { projectUrl: e.target.value })}
                          placeholder="https://myproject.com"
                          className="w-full border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2 bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        />
                      </div>
                      <div>
                        <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">GitHub URL (Optional)</label>
                        <input
                          type="url"
                          value={project.githubUrl}
                          onChange={(e) => updateProject(resume.id, project.id, { githubUrl: e.target.value })}
