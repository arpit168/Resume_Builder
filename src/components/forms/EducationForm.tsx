...
export function EducationForm({ resume }: { resume: Resume }) {
  const { addEducation, updateEducation, deleteEducation, updateResume } = useResume();
  const educations = resume.data.education;
  ...
  const moveUp = (index: number) => {
    ...
    updateResume(resume.id, { data: { ...resume.data, education: newArr } });
  };
      ...
      {educations.length === 0 ? (
        ...
        <div className="space-y-4">
          {educations.map((edu, index) => {
            ...
            return (
              <div key={edu.id} className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden bg-white dark:bg-gray-800 transition-all">
                {/* Header */}
                <div 
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="flex flex-col gap-1" onClick={(e) => e.stopPropagation()}>
                      ...
                      <button onClick={() => moveDown(index)} disabled={index === educations.length - 1} className="text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 disabled:opacity-30">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400">
                        {edu.institution ? `${edu.institution}${edu.location ? ` • ${edu.location}` : ''}` : "(Institution not specified)"}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button 
                      onClick={(e) => { e.stopPropagation(); deleteEducation(resume.id, edu.id); }}
                ...
                {isExpanded && (
                  <div className="p-4 space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Degree / Field of Study</label>
                        <input
                          type="text"
                          value={edu.degree}
                      ...
                      <div>
                        <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Grade / GPA</label>
                        <input
                          type="text"
                          value={edu.grade}
                      ...
                      <div>
                        <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">End Date (or expected)</label>
                        <input
                          type="month"
                          value={edu.endDate}
