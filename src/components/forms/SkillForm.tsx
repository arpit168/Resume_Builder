      ...
      {skills.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {skills.map((skill, index) => (
            <div key={skill.id} className="flex flex-col gap-2 p-3 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 shadow-sm group">
              <div className="flex items-center gap-2">
                <div className="flex flex-col -ml-1">
                  ...
                  <button onClick={() => moveDown(index)} disabled={index === skills.length - 1} className="text-gray-300 hover:text-gray-600 dark:hover:text-gray-400 disabled:opacity-30">
              ...
              <div className="pl-6">
                <select
                  value={skill.level}
                  onChange={(e) => updateSkill(resume.id, skill.id, { level: e.target.value })}
