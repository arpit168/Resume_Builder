...
const PROFICIENCY_LEVELS = ["Native/Bilingual", "Fluent", "Proficient", "Intermediate", "Basic"];

export function LanguageForm({ resume }: { resume: Resume }) {
  const { addLanguage, updateLanguage, deleteLanguage, updateResume } = useResume();
      ...
      {languages.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {languages.map((lang, index) => (
            <div key={lang.id} className="flex flex-col gap-2 p-3 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 shadow-sm group">
              <div className="flex items-center gap-2">
                <div className="flex flex-col -ml-1">
                  <button onClick={() => moveUp(index)} disabled={index === 0} className="text-gray-300 hover:text-gray-600 dark:hover:text-gray-400 disabled:opacity-30">
                    <ChevronUp className="w-4 h-4" />
                  </button>
                  <button onClick={() => moveDown(index)} disabled={index === languages.length - 1} className="text-gray-300 hover:text-gray-600 dark:hover:text-gray-400 disabled:opacity-30">
              ...
              <div className="pl-6">
                <select
                  ...
                  onChange={(e) => updateLanguage(resume.id, lang.id, { proficiency: e.target.value })}
